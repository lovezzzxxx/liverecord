...
EXCEPT_STREAM_PART_URL="${13:-noexcept}"
...
while true; do
	while true; do
		if [[ "${1}" == "youtube" || "${1}" == "youtubeffmmpeg" ]]; then
			...
			(curl -s "https://www.youtube.com/channel/${PART_URL}" | grep -q '<span class="yt-badge  yt-badge-live" >Live now</span>') && break
		fi
		...
		if [[ "${1}" == "bilibili" ]]; then
			...
			if (curl -s "https://api.live.bilibili.com/room/v1/Room/get_info?room_id=${PART_URL}" | grep -q '"live_status":1'); then
				if [[ "${EXCEPT_YOUTUBE_PART_URL}" != "noexcept" ]]; then
					...
					(curl -s "https://www.youtube.com/channel/${EXCEPT_YOUTUBE_FULL_URL}" | grep -q '<span class=\"yt-badge  yt-badge-live\" >Live now</span>') && echo "${LOG_PREFIX} ${EXCEPT_YOUTUBE_FULL_URL} is restream now. retry after ${INTERVAL} seconds..." && sleep ${INTERVAL} && continue
				fi
	...
	if [[ "${1}" == "youtubeffmmpeg" ]]; then STREAM_URL=$(streamlink --stream-url "${FULL_URL}" "${FORMAT}") ; METADATA=$(youtube-dl --get-id --get-title --get-description --no-playlist --playlist-items 1 --match-filter is_live "${FULL_URL}" 2>/dev/null) ; FNAME="youtube_${PART_URL}_$(date +"%Y%m%d_%H%M%S")_$(echo "${METADATA}" | sed -n '2p').ts" ; echo "${METADATA}" > "${DIR_LOCAL}/${FNAME}.log"; fi
	...
	if [[ "${LOOPORTIME}" == "once" || "${LOOPORTIME}" == "loop" ]]; then
		...
		if [[ "${1}" == "youtubeffmmpeg" || "${1}" == "twitcastffmpeg" || "${1}" == "twitch" || "${1}" == "openrec" || "${1}" == "mirrativ" || "${1}" == "bilibili" || "${1}" == "streamlink" || "${1}" == "m3u8" ]]; then
