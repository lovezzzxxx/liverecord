...
while true; do
		...
		if [[ "${1}" == "bilibili"* ]]; then
			...
			if (wget -q -O- "https://api.live.bilibili.com/room/v1/Room/get_info?room_id=${PART_URL}" | grep -q '"live_status":1'); then
				if [[ "${EXCEPT_YOUTUBE_PART_URL}" != "noexcept" ]]; then
					...
					#(wget -q -O- "${EXCEPT_YOUTUBE_FULL_URL}" | grep -q '\\"playabilityStatus\\":{\\"status\\":\\"OK\\"') && echo "${LOG_PREFIX} ${EXCEPT_YOUTUBE_FULL_URL} is restream now. retry after ${LOOPINTERVAL} seconds..." && sleep ${LOOPINTERVAL} && continue
				fi
	...
	if [[ "${LOOPORTIME}" == "once" || "${LOOPORTIME}" == "loop" ]]; then
		LOG_PREFIX=$(date +"[%Y-%m-%d %H:%M:%S]")
		echo "${LOG_PREFIX} record start ${STREAM_URL}" #开始录制
		if [[ "${1}" == "youtube" ]]; then
		...
		LOG_PREFIX=$(date +"[%Y-%m-%d %H:%M:%S]")
		echo "${LOG_PREFIX} record start pid=${RECORDPID} stoptimestamp=${RECORDSTOPTIME} ${STREAM_URL}" #开始录制
		while true; do
			sleep 15
