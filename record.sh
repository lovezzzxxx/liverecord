...
while true; do
	...
	if [[ "${1}" == "youtube" || "${1}" == "youtubeffmpeg" ]]; then ID=$(wget -q -O- "${FULL_URL}" | grep -o '\\"videoId\\":\\".*\\"' | head -n 1 | sed 's/\\//g' | awk -F'"' '{print $4}') ; FNAME="youtube_${PART_URL}_$(date +"%Y%m%d_%H%M%S")_${ID}.ts"; fi
