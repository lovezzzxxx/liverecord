...
EXCEPT_MIRRATIVE_PART_URL="${14:-noexcept}"
EXCEPT_REALITY_PART_URL="${15:-noexcept}"
EXCEPT_STREAM_PART_URL="${16:-noexcept}"
...
while true; do
	while true; do
		...
		if [[ "${1}" == "17live" ]]; then
			LOG_PREFIX=$(date +"[%Y-%m-%d %H:%M:%S]")
			echo "${LOG_PREFIX} metadata ${FULL_URL}"
			STREAM_URL=$(curl -s -X POST 'http://api-dsa.17app.co/api/v1/liveStreams/getLiveStreamInfo' --data "{\"liveStreamID\": ${PART_URL}}" | grep -o '\\"webUrl\\":\\"[^\\]*' | awk -F'\"' '{print $4}')
			[[ -n "${STREAM_URL}" ]] && break
		fi
		...
		if [[ "${1}" == "bilibili" ]]; then
			...
			if (curl -s "https://api.live.bilibili.com/room/v1/Room/get_info?room_id=${PART_URL}" | grep -q '"live_status":1'); then
				...
				if [[ "${EXCEPT_NICOCO_PART_URL}" != "noexcept" ]]; then
					...
					EXCEPT_NICOCO_LIVE_URL=$(curl -s "https://com.nicovideo.jp/community/${EXCEPT_NICOCO_PART_URL}" | grep -o '<a class="now_live_inner" href="https://live.nicovideo.jp/watch/lv[0-9]*' | head -n 1 | awk -F'"?' '{print $4}')
				...
				if [[ "${EXCEPT_NICOCH_PART_URL}" != "noexcept" ]]; then
					...
					EXCEPT_NICOCH_LIVE_URL=$(curl -s "https://ch.nicovideo.jp/${EXCEPT_NICOCH_PART_URL}/live" | awk 'BEGIN{RS="<section class=";FS="\n";ORS="\n";OFS="\t"} $1 ~ /sub now/ {LIVE_POS=match($0,"https://live.nicovideo.jp/watch/lv[0-9]*");LIVE=substr($0,LIVE_POS,RLENGTH);print LIVE}' | head -n 1)
					...
					EXCEPT_NICOCH_LIVE_ID=$(curl -s "https://ch.nicovideo.jp/${EXCEPT_NICOCH_PART_URL}" | grep -o "data-live_id=\"[0-9]*\" data-live_status=\"onair\"" | head -n 1 | awk -F'"' '{print $2}') ; LIVE_URL="https://live.nicovideo.jp/watch/lv${LIVE_ID}"
				...
				if [[ "${EXCEPT_17LIVE_PART_URL}" != "noexcept" ]]; then
					...
					EXCEPT_17LIVE_STREAM_URL=$(curl -s -X POST 'http://api-dsa.17app.co/api/v1/liveStreams/getLiveStreamInfo' --data "{\"liveStreamID\": ${EXCEPT_17LIVE_PART_URL}}" | grep -o '\\"webUrl\\":\\"[^\\]*' | awk -F'\"' '{print $4}')
					[[ -n "${EXCEPT_17LIVE_STREAM_URL}" ]] && echo "${LOG_PREFIX} ${EXCEPT_17LIVE_FULL_URL} is restream now. retry after ${INTERVAL} seconds..." && sleep ${INTERVAL} && continue
				fi
	...
	if [[ "${LOOPORTIME}" == "once" || "${LOOPORTIME}" == "loop" ]]; then
		...
		if [[ "${1}" == "youtubeffmpeg" || "${1}" == "youtubecurlffmpeg" || "${1}" == "twitcastffmpeg" || "${1}" == "twitch" || "${1}" == "openrec" || "${1}" == "mirrativ" || "${1}" == "reality" || "${1}" == "17live" || "${1}" == "bilibili" || "${1}" == "streamlink" || "${1}" == "m3u8" ]]; then
	...
	if [[ ! -f "${DIR_LOCAL}/${FNAME}" ]]; then #判断是否无录像
		...
		if [[ "${BACKUP}" == "baidupan"* || "${BACKUP}" == "both"* ]]; then #上传baidupan
			...
			(echo "${ERRFLAG_BAIDUPAN_FILE}" | grep -q "成功") && echo "${LOG_PREFIX} upload baidupan ${DIR_LOCAL}/${FNAME} success"
			...
			(echo "${ERRFLAG_BAIDUPAN_FILE}" | grep -q "成功" || echo "${ERRFLAG_BAIDUPAN_FILE}" | grep -q "未检测到上传的文件") || (echo "${LOG_PREFIX} upload baidupan ${DIR_LOCAL}/${FNAME} fail" && echo "${LOG_PREFIX} upload baidupan ${DIR_LOCAL}/${FNAME} fail" > "${DIR_LOCAL}/${FNAME}.baidupan_fail_error.log")
			...
			(echo "${ERRFLAG_BAIDUPAN_LOG}" | grep -q "成功") && echo "${LOG_PREFIX} upload baidupan ${DIR_LOCAL}/${FNAME}.log success"
			...
			(echo "${ERRFLAG_BAIDUPAN_LOG}" | grep -q "成功" || echo "${ERRFLAG_BAIDUPAN_LOG}" | grep -q "未检测到上传的文件") || (echo "${LOG_PREFIX} upload baidupan ${DIR_LOCAL}/${FNAME}.log fail" && echo "${LOG_PREFIX} upload baidupan ${DIR_LOCAL}/${FNAME}.log fail" > "${DIR_LOCAL}/${FNAME}.log.baidupan_fail_error.log")
		fi
